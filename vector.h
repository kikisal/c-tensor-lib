...
vector vector_from_array(void* arr, size_t element_size, size_t count);
...
vector vector_clone(vector v) {
	...
	internal_vector* src = v;

	if (vec) {
		vec->items = malloc(sizeof(*src->items) * src->size); /* Allocate 64 bytes (8 array allocations) to the vector. */
		if (!vec->items) return NULL;

		vec->capacity = src->capacity;
		vec->size     = src->size;
		memcpy(vec->items, src->items, vec->size * sizeof(*src->items));
		return vec;
	}
...
vector vector_from_array(void* arr, size_t element_size, size_t count) {
	...
	for (int i = 0; i < count; ++i) {
		uint8_t* ptr = arr;
		vector_push_back(v, ptr + i*element_size, element_size);
	}
